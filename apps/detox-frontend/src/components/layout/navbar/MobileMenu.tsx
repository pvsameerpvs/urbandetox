...
export function MobileMenu({ isLightMode }: MobileMenuProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent side="right" showCloseButton={false} className="w-80 bg-white p-0 border-l border-border/40">
        <SheetTitle className="sr-only">Navigation Menu</SheetTitle>

        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-border/30">
          <Link href="/" onClick={() => setOpen(false)} className="inline-block">
            <Image src="/log-detox.png" alt="Urban Detox" width={130} height={36} className="h-8 w-auto object-contain" priority />
          </Link>
          <button onClick={() => setOpen(false)} className="p-2 rounded-lg hover:bg-secondary transition-colors" aria-label="Close menu">
          ...
          <div className="px-5 py-4 border-b border-border/30 bg-secondary/20">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button variant="outline" size="sm" className="h-9 rounded-lg text-xs font-medium" asChild>
                <Link href="/profile" onClick={() => setOpen(false)}><User className="mr-1.5 h-3.5 w-3.5" /> Profile</Link>
              </Button>
              <Button size="sm" className="h-9 rounded-lg bg-brand text-brand-foreground hover:bg-brand/90 text-xs font-medium" asChild>
                <Link href="/my-detox" onClick={() => setOpen(false)}><MapPin className="mr-1.5 h-3.5 w-3.5" /> My Detox</Link>
              </Button>
            </div>
          </div>
        )}
        ...
        {/* Nav Links */}
        <nav className="p-3">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-2 px-3">Menu</p>
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
        ...
        <div className="px-5 py-3 border-t border-border/30">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground mb-2">Popular Detoxes</p>
        ...
        {/* Footer CTA */}
        <div className="mt-auto p-5 border-t border-border/30 bg-secondary/20">
          {isHydrated && isLoggedIn ? (
            <Button variant="ghost" onClick={handleLogout} className="w-full h-11 rounded-xl text-destructive hover:text-destructive hover:bg-destructive/10 text-sm font-medium">
              <LogOut className="mr-2 h-4 w-4" /> Log Out
            </Button>
          ) : (
            <>
              <Button className="w-full h-11 rounded-xl bg-brand text-brand-foreground hover:bg-brand/90 text-sm font-semibold shadow-sm" asChild>
                <Link href="/login" onClick={() => setOpen(false)}>
                  <LogIn className="mr-2 h-4 w-4" /> Log In
                </Link>
              </Button>
              <p className="text-center text-[11px] text-muted-foreground mt-3">
                Or <Link href="/detox" onClick={() => setOpen(false)} className="text-brand hover:underline font-medium">book a detox first</Link>
              </p>
            </>
          )}
        </div>
