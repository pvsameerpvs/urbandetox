...
export default function BookingPage() {
  ...
  return (
    <main className="min-h-screen bg-white pb-24 md:pb-0">
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="lg:col-span-3 space-y-6 sm:space-y-8">
            ...
            <motion.div variants={itemVariants}>
              <Card className="border-0 shadow-lg shadow-black/[0.03] bg-white rounded-2xl">
                <CardContent className="p-4 sm:p-5 md:p-6">
                  <TravelerCounter count={travelerCount} maxSeats={maxSeats} pricePerPerson={currentPrice} onChange={(count) => {
                    if (count > travelers.length) {
                      const added = Array.from({ length: count - travelers.length }, (_, i) => createCompanionTraveler(i));
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {travelerCount === 1
                      ? (profile.personal.fullName ? "Pre-filled from your profile. Tap to edit." : "Tap to fill your details.")
                      : "Tap each card to fill details. Primary traveler is pre-filled from your profile."}
                  </p>
                </div>
              </div>
            ...
            <div className="hidden lg:block">
              <Button
                ...
                disabled={!allTravelersValid}
                className="w-full rounded-xl bg-brand text-brand-foreground hover:bg-brand/90 h-12 text-sm font-semibold shadow-lg shadow-brand/10 disabled:opacity-50"
              >
                Continue to Payment <ChevronRight className="ml-2 h-4 w-4" />
              </Button>
              {!allTravelersValid && (
                <p className="text-xs text-amber-600 text-center mt-2">{incompleteCount} traveler{incompleteCount > 1 ? "s" : ""} need name and phone</p>
              )}
            </div>
          </motion.div>
      ...
      <MobileBookingCTA
        total={grandTotal}
        label={allTravelersValid ? "Continue to Payment" : `Fill ${incompleteCount} traveler${incompleteCount > 1 ? "s" : ""}`}
        onClick={handleSubmit}
        disabled={!allTravelersValid}
      />
    </main>
