...
export function UpcomingDetoxes({ departures, packages }: Props) {
  ...
  const upcoming = useMemo(
    () =>
      departures
        .filter((d) => d.tripStatus !== 'completed' && new Date(d.startDate) >= today)
  ...
  const TRIP_PALETTE = [
    { bg: 'rgba(200,241,107,0.4)', color: 'var(--forest-deep)', activeBg: 'var(--forest-deep)', activeColor: 'var(--lime)' }, // Lime/Forest
    { bg: 'rgba(74,157,148,0.3)', color: '#2C5F59', activeBg: '#2C5F59', activeColor: '#E6F3F1' }, // Teal
    { bg: 'rgba(217,163,114,0.3)', color: '#6B4423', activeBg: '#6B4423', activeColor: '#FFF4EB' }, // Amber/Earth
    { bg: 'rgba(122,158,126,0.3)', color: '#2D4B31', activeBg: '#2D4B31', activeColor: '#F2F7F3' }, // Sage
    { bg: 'rgba(164,113,153,0.25)', color: '#5A2E4E', activeBg: '#5A2E4E', activeColor: '#FDF4FA' }, // Plum/Dusty
  ];
  ...
  function getTripColor(depId: string, isActive: boolean) {
    ...
    return isActive ? { bg: theme.activeBg, text: theme.activeColor } : { bg: theme.bg, text: theme.color };
  }
  ...
  const scrollCards = (dir: 'left' | 'right') => {
    ...
    const amount = 320;
    cardsRef.current.scrollBy({ left: dir === 'right' ? amount : -amount, behavior: 'smooth' });
  };
      ...
      <div className="container">
        ...
        >
          ...
          >
            {months.map(({ year, month, days }, idx) => (
              <div key={`${year}-${month}`} style={{ width: '250px', flexShrink: 0 }}>
                      ...
                      >
                        ...
                        >
                          {dayTrips.map((dep) => {
                            ...
                            const isTripInActiveRange = activeDate && (activeDate >= dep.startDate.slice(0, 10) && activeDate <= dep.endDate.slice(0, 10));
                            const colors = getTripColor(dep.id, !!isTripInActiveRange);
                        ...
                        <span
                          style={{
                            ...
                            color: dayTrips.some(d => {
                              ...
                              return activeDate && activeDate >= s && activeDate <= e;
                            })
                              ? 'white'
                              : 'var(--forest-deep)',
                            pointerEvents: 'none',
        ...
        <div
          ...
          style={{
            ...
            gridTemplateColumns: activeDate ? 'repeat(auto-fill, minmax(min(100%, 280px), 1fr))' : undefined,
            gap: '0.75rem',
        ...
        >
          {visibleDeps.length === 0 ? (
            ...
            visibleDeps.map((dep, idx) => {
              ...
              const startLabel = new Date(dep.startDate).toLocaleDateString('en-IN', {
                day: 'numeric', month: 'short',
              });
              const endLabel = new Date(dep.endDate).toLocaleDateString('en-IN', {
                day: 'numeric', month: 'short',
              });
                ...
                >
                  ...
                  <div style={{ position: 'relative', height: '130px', overflow: 'hidden' }}>
                    ...
                    <div
                      style={{
                        position: 'absolute',
                        inset: 0,
                        background: 'linear-gradient(to top, rgba(8,30,20,0.55) 0%, transparent 60%)',
                      }}
                    />
                  ...
                  <div style={{ padding: '0.65rem 0.85rem 0.85rem' }}>
                    ...
                    <div style={{ marginBottom: '1rem' }}>
                      ...
                      >
                        <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.65rem', color: seatsLeft <= 3 ? '#e05c2e' : 'var(--text-light)', fontWeight: seatsLeft <= 3 ? 700 : 400 }}>
                          {seatsLeft} {seatsLeft === 1 ? 'seat' : 'seats'} left
                        </span>
                        <span style={{ fontFamily: 'var(--font-inter)', fontSize: '0.65rem', color: 'var(--text-light)' }}>
                    ...
                    >
                      <div>
                        <p style={{ fontFamily: 'var(--font-inter)', fontSize: '0.62rem', color: 'var(--text-light)', margin: 0 }}>per person</p>
                      ...
                      <span
                        style={{
                          ...
                          fontSize: '0.72rem',
                          fontWeight: 400,
                          background: isHovered ? 'var(--lime)' : 'var(--forest-deep)',
        ...
        <div style={{ textAlign: 'center', marginTop: '2rem' }}>
          ...
          >
            ...
            <svg width="12" height="12" viewBox="0 0 16 16" fill="none">
              <path d="M2 8h12M10 4l4 4-4 4" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </Link>
        </div>
      </div>
